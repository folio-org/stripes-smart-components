...
class ChangeDueDate extends React.Component {
  ...
  async changeDueDate() {
    ...
    // Previously, this function used Promise.all() to execute these loan changes
    // in parallel. But this was overloading the backend, where the transactions involved
    // are complex, and causing many of the requests to time out with 500 errors. We're
    // switching to sequential execution here in an attempt to keep the backend happy.
    for (let i = 0; i < loans.length; i++) {
      // Chaining a catch after the performLoanChange op for each loan
      // traps the errors that the server returns if there's an
      // 'unproccessable entity', e.g. because the item is declared
      // lost. This allows us to resolve errors with a user-friendly message
      // once updating is done.
      // eslint-disable-next-line no-await-in-loop
      const result = await this.performLoanChange(loans[i]).catch(e => e);
      results.push(result);
    }
