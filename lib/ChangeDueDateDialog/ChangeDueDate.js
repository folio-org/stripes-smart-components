...
class ChangeDueDate extends React.Component {
  ...
  handleDateTimeChanged(datetime) {
    const warnings = [];
    const formatMessage = this.props.stripes.intl.formatMessage;

    if (this.userWillBeExpired(datetime)) {
      warnings.push(formatMessage({ id: 'stripes-smart-components.cddd.warning.dueDateAfterPatronExpiration' }));
    }

    if (this.newDueDateIsInThePast(datetime)) {
      warnings.push(formatMessage({ id: 'stripes-smart-components.cddd.warning.dueDateInPast' }));
    }
