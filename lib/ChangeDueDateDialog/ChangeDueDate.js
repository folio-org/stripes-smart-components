...
class ChangeDueDate extends React.Component {
  ...
  //  actual loan objects are in props.
  onlyLostItems(checkboxLoanIds) {
    var selectedLoanIds = Object.keys(checkboxLoanIds).filter(id => checkboxLoanIds[id] == true);
    var selectedLoans = this.props.loans.filter(loan => selectedLoanIds.includes(loan.id));

    return !Object.values(selectedLoans).find(loan => loan?.item?.status?.name !== 'Declared lost');
  }
