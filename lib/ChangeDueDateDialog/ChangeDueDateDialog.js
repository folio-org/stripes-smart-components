...
class ChangeDueDateDialog extends React.Component {
  ...
  itemIsDeclaredLost(loanId) {
    const matchedLoan = this.loans().find(loan => loanId === loan.id);
    return matchedLoan?.item?.status?.name === 'Declared lost';
  }
  ...
  handleDueDateChanged(changeResults) {
    ...
    changeResults.forEach(result => {
      if (result.id) {
        ...
        // There is a specific error message for items declared lost,
        ...
        const errorMessage = this.itemIsDeclaredLost(failedLoanId)
          ? 'stripes-smart-components.cddd.itemDeclaredLost'
          : 'stripes-smart-components.cddd.changeFailed';
    ...
    this.setState({
      succeeded: true,
      dueDatesChanged: changeResults.length,
      alerts,
    });
