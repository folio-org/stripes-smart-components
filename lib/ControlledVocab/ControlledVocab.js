...
class ControlledVocab extends React.Component {
  ...
  showDeletionSuccessCallout(item) {
    // FIXME: Convert to whatever comes out of STRIPES-518
    const message = (
  ...
  renderItemInUseDialog() {
    const term = this.props.labelSingular.toLowerCase();

    return (
      <Modal
        open={this.state.showItemInUseDialog}
        label={this.props.stripes.intl.formatMessage({ id: 'stripes-smart-components.cv.cannotDeleteTermHeader' }, { term })}
        size="small"
      >
        <Row>
          <Col xs>
            <FormattedMessage id="stripes-smart-components.cv.cannotDeleteTermMessage" values={{ term }} />
          </Col>
        </Row>
        <Row>
          <Col xs>
  ...
  render() {
    ...
    const modalMessage = <span>The {this.props.labelSingular.toLowerCase()} <strong>{this.state.selectedItem[this.state.primaryField]}</strong> will be <strong>deleted.</strong></span>;
      ...
      <Paneset>
        <Pane defaultWidth="fill" fluidContentWidth paneTitle={this.props.label}>
          ...
          <ConfirmationModal
            ...
            heading={formatMessage({ id: 'stripes-core.button.deleteEntry' }, { entry: this.props.labelSingular.toLowerCase() })}
