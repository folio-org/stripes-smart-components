...
describe('ControlledVocab', () => {
  ...
  describe('dismissPane property', () => {
    describe('when not provided', () => {
      beforeEach(async () => {
        await mountComponent(true, server, {});
      });

      it('should not render a dismissal button', () => IconButton({ icon: 'times' }).absent());
    });

    describe('when provided', () => {
      beforeEach(async () => {
        await mountComponent(true, server, { dismissPane: () => {} });
      });

      it('should render a dismissal button', () => IconButton({ icon: 'times' }).exists());
    });
  });
});
