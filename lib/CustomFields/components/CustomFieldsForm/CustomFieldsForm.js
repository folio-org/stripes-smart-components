...
const CustomFieldsForm = ({
  ...
  const getFieldTypesOptions = () => {
    return Object.values(fieldTypes)
      .map(fieldType => ({
        labelElement: fieldTypesLabels[fieldType],
        labelText: intl.formatMessage({ id: fieldTypesLabelIds[fieldType] }),
        value: fieldType,
      }))
      .sort((fieldA, fieldB) => fieldA.labelText.localeCompare(fieldB.labelText));
  };
