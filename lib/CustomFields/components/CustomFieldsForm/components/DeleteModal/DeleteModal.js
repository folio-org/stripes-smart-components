...
const propTypes = {
  ...
  optionsToDelete: PropTypes.object.isRequired,
...
const DeleteModal = ({
  ...
  const renderOptionsDeleteList = () => Object.values(optionsToDelete).map(options => options.map(option => {
    const {
      fieldName,
      optionData
    } = option;
    ...
    return (
      <div key="key" data-test-custom-field-option-delete-warning>
        <SafeHTMLMessage
