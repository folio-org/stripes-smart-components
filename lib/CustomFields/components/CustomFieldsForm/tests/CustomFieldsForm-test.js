...
describe('CustomFieldsForm', () => {
  ...
  describe.skip('when clicking "Expand all"', () => {
    beforeEach(async () => {
      await customFieldsForm.whenLoaded();
      await customFieldsForm.expandAllButton.clickExpandAllButton();
    });

    it('should expand all accordions', () => {
      for (let i = 0; i < customFieldsForm.customFieldAccordions().length; i++) {
        expect(customFieldsForm.customFieldAccordions(i).isOpen).to.be.true;
      }
    });
  });
  ...
  describe('when clicking on an accordion', () => {
    ...
    beforeEach(async () => {
      ...
      customFieldsForm.customFieldAccordions(0).isOpen;
  ...
  describe('when deleting a custom field', () => {
    ...
    describe.skip('when cancelling deletion', () => {
      beforeEach(async () => {
        await customFieldsForm.deleteModal.whenLoaded();
        await customFieldsForm.deleteModal.cancelButton.click();
      });

      it('should call cancel delete handler', () => {
        expect(onCancelDelete.calledOnce).to.be.true;
      });

      it('should still show the field', () => {
        expect(customFieldsForm.customFieldAccordions().length).to.equal(1);
      });
    });

    describe.skip('when confirming deletion', () => {
      beforeEach(async () => {
        await customFieldsForm.deleteModal.whenLoaded();
        await customFieldsForm.deleteModal.confirmButton.click();
      });

      it('should call confirm delete handler', () => {
        expect(onConfirmDelete.calledOnce).to.be.true;
      });
    });
  });
});
