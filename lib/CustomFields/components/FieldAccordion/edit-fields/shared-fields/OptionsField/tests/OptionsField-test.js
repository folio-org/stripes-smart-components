...
describe.only('OptionsField', () => {
  ...
  describe('when field type is multi select', () => {
    beforeEach(async () => {
      await renderComponent({
        isMultiSelect: true,
        defaultOptions: ['Science', 'Economics'],
      }, {
        initialValues: {
          customFields: [
            {
    ...
    it('should check correct default checkboxes', () => {
      const statuses = [
        optionsField.rows(0).defaultChecked,
        optionsField.rows(1).defaultChecked,
        optionsField.rows(2).defaultChecked,
      ];
