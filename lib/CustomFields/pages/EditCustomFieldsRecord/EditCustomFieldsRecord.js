...
// formField(MultiSelection) computes error as (meta.touched && meta.error), but removing
// items never triggers onBlur so touched stays false. Passing error in rest overrides
// the config-computed value; use meta.dirty so errors show as soon as the value changes.
const MultiSelectionWithDirtyError = ({ input, meta, ...rest }) => {
  const Comp = fieldComponents[fieldTypes.MULTISELECT];
  return <Comp input={input} meta={meta} {...rest} error={(meta?.dirty || meta?.touched) && meta?.error ? meta.error : ''} />;
};
...
const EditCustomFieldsRecord = ({
  ...
  const renderCustomField = customField => {
    if (customField.type === fieldTypes.SELECT) {
      ...
      return createCustomFieldRenderFunction(customField)({
        component: MultiSelectionWithDirtyError,
        dataOptions: collectDataOptionsForSelect(customField),
