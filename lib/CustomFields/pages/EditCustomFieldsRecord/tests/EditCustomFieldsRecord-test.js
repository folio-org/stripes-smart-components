...
describe('EditCustomFieldsRecord', () => {
  ...
  describe('when user clears single select field with default value', () => {
    let formApi;
    ...
    beforeEach(async () => {
              ...
              <FormSpy subscription={{ values: true }}>
                {({ values }) => (
                  <EditCustomFieldsRecord
                    ...
                    finalFormCustomFieldsValues={values.customFields}
                    entityType="user"
                    expanded
                    formName="custom-fields-test"
                    isReduxForm={false}
      ...
      await converge(() => {
        ...
        expect(fieldValue).to.equal('opt_0');
      });

      await Select('Single select').choose('Select a single select');
    });

    it('should keep the field empty and not reset to default value', () => {
      return converge(() => {
        const fieldValue = formApi.getState().values.customFields?.['single_select-1'];
