...
const EditCustomFieldsSettings = ({
  ...
  const handleOptionDelete = fieldId => optionData => {
    ...
    const isOptionSaved = customField.selectField.options.values.find(option => optionData.id === option.id);

    if (isOptionSaved) {
      setOptionsToDelete(current => {
        const newOptionsToDelete = { ...current };

        if (fieldId in optionsToDelete) {
          newOptionsToDelete[fieldId] = [
            ...newOptionsToDelete[fieldId],
            {
