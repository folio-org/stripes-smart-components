...
describe('EditCustomFieldsSettings', () => {
  ...
  const viewRoute = '/custom-fields-view';
  ...
  describe('when editing section title', () => {
    ...
    describe('and clicking Cancel', () => {
      ...
      it('should redirect to the view route', function () {
        return converge(() => {
          if (!this.location.pathname.includes(viewRoute)) {
            throw new Error(`pathname does not include "${viewRoute}"`);
          }
        });
      });
    });
    ...
    describe('filling options for new field', () => {
      ...
      it('should send correct new custom field data', () => {
        return converge(() => {
          ...
          if ('displayInAccordion' in body.customFields[7]) {
            throw new Error(`expected body to not have displayInAccordion, got ${displayInAccordion}`);
          }
        });
      });
  ...
  describe('when the `hasDisplayInAccordionField` prop is true', () => {
    ...
    it('should display the "Display in accordion" field', () => Select('Display in accordion').exists());

    it('should display correct value for the "Display in accordion" field', () => {
      return Select('Display in accordion').has({ checkedOptionText: 'Fees/Fines' });
    });

    it('should display correct options for the "Display in accordion" field', () => {
      return Select('Display in accordion').has({ optionsText: ['Custom Fields label', 'Fees/Fines'] });
    });

    describe('when updating a field and saving', () => {
      beforeEach(async () => {
        await CFAccordionInteractor({ index: 0 }).find(TextField('Field label*')).fillIn('alpha');
        await saveButton.click();
      });

      it('should send correct `displayInAccordion` field value', () => {
        return converge(() => {
          const body = JSON.parse(updateCustomFieldsHandler.args[0][1].requestBody);
          const firstField = body.customFields[0];
          const secondField = body.customFields[1];

          if (firstField.displayInAccordion !== 'feesFines') {
            throw new Error(`expected firstField displayInAccordion to be 'feesFines', got ${firstField.displayInAccordion}`);
          }

          if (secondField.displayInAccordion !== 'default') {
            throw new Error(`expected secondField displayInAccordion to be 'default', got ${secondField.displayInAccordion}`);
          }

          if (body.customFields.some(field => !field.displayInAccordion)) {
            throw new Error('expected all fields to have displayInAccordion set, but found some without it');
          }
        });
      });
    });

    describe('when adding a new field and saving', () => {
      beforeEach(async () => {
        await form.addField('Text field');
        await CFAccordionInteractor({ index: 7 }).find(TextField('Field label*')).fillIn('New text field');
        await saveButton.click();
      });

      it('should send `displayInAccordion` field', () => {
        return converge(() => {
          const body = JSON.parse(updateCustomFieldsHandler.args[0][1].requestBody);

          if (body.customFields[7].displayInAccordion !== 'default') {
            throw new Error('expected displayInAccordion to have "default" as its default value');
          }
        });
      });
    });
