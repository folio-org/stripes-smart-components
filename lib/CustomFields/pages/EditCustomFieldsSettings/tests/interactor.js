...
export default interactor(class EditCustomFieldsSettings {
  ...
  expand = clickable('[data-tast-expand-button]');
  ...
  deleteButtonsArePresent = isPresent('[data-test-delete-option]');
  areCustomFieldsPresent = isPresent('[data-test-accordion-section]');
  deleteModalPresent = isPresent('[data-test-delete-custom-fields-modal]');
  addFieldButton = new AddButtonInteractor();
  deleteModal = new DeleteModalInteractor();

  whenDeleteModalIsLoaded() {
    return this.when(() => this.deleteModalPresent, 5000);
  }

  whenDeleteButtonsArePresent() {
    return this.when(() => this.deleteButtonsArePresent, 5000);
  }

  whenDeleteModalIsHidden() {
    return this.when(() => !this.deleteModalPresent, 5000);
  }
