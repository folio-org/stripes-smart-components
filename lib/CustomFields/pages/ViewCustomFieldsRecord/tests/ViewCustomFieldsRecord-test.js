...
describe('ViewCustomFieldsRecord', () => {
  ...
  describe('when the `sectionId` is "feesFines"', () => {
    ...
    it('should not display an accordion', () => Accordion().absent());
  });
  ...
  describe('when allowedRefIds is provided', () => {
    beforeEach(async () => {
      await renderComponent({
        allowedRefIds: ['textbox-1', 'single_select-1'],
      });
    });

    it('should display only the allowed custom fields', () => viewCustomFields.has({ fieldCount: 2 }));
  });

  describe('when showAccordion is false', () => {
    beforeEach(async () => {
      await renderComponent({
        showAccordion: false,
      });
