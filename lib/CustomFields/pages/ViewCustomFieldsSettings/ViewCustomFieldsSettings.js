...
const ViewCustomFieldsSettings = ({ redirectToEdit, okapi, backendModuleId, entityType, backendModuleName, permissions }) => {
  ...
  const renderLastMenu = () => (
    <IfPermission perm={permissions.canEdit}>
      <Button
        buttonStyle="primary"
        marginBottom0
  ...
  const renderSectionTitle = () => (
    <KeyValue label={<FormattedMessage id="stripes-smart-components.customFields.settings.sectionTitle" />}>
      <span className={styles.sectionTitle}>
        {sectionTitle.value ?? <FormattedMessage id="stripes-smart-components.customFields.recordAccordion.defaultName" />}
      </span>
    </KeyValue>
  );
...
)(stripesConnect(ViewCustomFieldsSettings));
