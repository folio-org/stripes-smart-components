...
describe('ViewCustomFieldsSettings', () => {
  ...
  describe('when custom field is single select', () => {
    ...
    it('should disable checkboxes', () => {
      const disabledStates = viewCustomFields.customFields(2).fields(2).options().map(option => option.columns(1).inputDisabled);
  ...
  describe('when custom field is multi select', () => {
    ...
    it('should check default options', () => {
      const checkedStates = viewCustomFields.customFields(3).fields(2).options().map(option => option.columns(1).inputChecked);

      expect(checkedStates).to.deep.equal([true, true, false]);
    });

    it('should disable checkboxes', () => {
      const disabledStates = viewCustomFields.customFields(3).fields(2).options().map(option => option.columns(1).inputDisabled);
  ...
  describe('when custom field is radiobutton set', () => {
    ...
    it('should disable checkboxes', () => {
      const disabledStates = viewCustomFields.customFields(4).fields(1).options().map(option => option.columns(1).inputDisabled);
