...
class EditableListForm extends React.Component {
  ...
  onSave(fields, index) {
    ...
    Promise.resolve(res).then(
      ...
      async (response) => this.setError(index, await processBadResponse(response, defaultErrorCodes.defaultSaveError))
    );
  }
  ...
  onDelete(fields, index) {
    ...
    Promise.resolve(res).then(
      ...
      async (response) => this.setError(index, await processBadResponse(response, defaultErrorCodes.defaultRemoveError))
    );
  }
    ...
    if (status[item.rowIndex].editing) {
      return (
        <div style={{ display: 'flex' }}>
          <Button
            disabled={pristine || submitting || invalid || creating}
            marginBottom0
