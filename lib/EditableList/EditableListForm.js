...
const propTypes = {
  ...
  */
  ...
  /**
   * Boolean value which indicates that form should be reinitialized
   * after changing of input parameters
   */
  shouldReinitialize: PropTypes.bool,
  /**
  ...
  /**
   * Should user be asked to confirm delete action
   */
...
const defaultProps = {
  ...
  confirmationCancelLabel: <FormattedMessage id="stripes-smart-components.editableList.confirmationModal.cancelLabel" />,
  confirmationConfirmLabel: <FormattedMessage id="stripes-smart-components.editableList.confirmationModal.confirmLabel" />,
...
class EditableListForm extends React.Component {
  ...
  shouldUpdateStatus(previousValues) {
    return (
      this.props.initialValues &&
      previousValues &&
      !isEqual(this.props.initialValues, previousValues)
    );
  }
  ...
  handleDeleteClick = (fields, index) => {
    const {
    ...
    this.setState({
      ...
      itemIdToDelete: item[uniqueField],
    });
  }
  ...
  handleDeleteConfirm = (fields) => {
    ...
    const itemIndex = values.items.findIndex(item => item[uniqueField] === this.state.itemIdToDelete);
          ...
          <FormattedMessage id="stripes-components.deleteThisItem">
            {([ariaLabel]) => (
              <IconButton
                ...
                aria-label={ariaLabel}
                onClick={() => this.handleDeleteClick(fields, item.rowIndex)}
  ...
  renderConfirmationModal = (fields) => {
    const {
    ...
    return (
      <ConfirmationModal
        ...
        onConfirm={() => this.handleDeleteConfirm(fields)}
      />
    );
  }
  ...
  RenderItems({ fields }) {
    ...
    return (
      ...
      >
        ...
        {this.renderConfirmationModal(fields)}
      </HasCommand>
    );
  }
