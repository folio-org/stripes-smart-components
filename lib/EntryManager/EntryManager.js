...
class EntryManager extends React.Component {
  ...
  onSave(entry) {
    ...
    const dk = this.props.dataKey ? this.props.dataKey : 'entries';
    return this.props.parentMutator[dk][action](entry)
      .then(() => this.hideLayer());
  }

  onRemove(entry) {
    const dk = this.props.dataKey ? this.props.dataKey : 'entries';
    return this.props.parentMutator[dk].DELETE(entry).then(() => {
    ...
    const addMenu = (
      <IfPermission perm={permissions.post}>
        ...
        <IfPermission perm={permissions.put}>
          <PaneMenu>
            <Button title={`Add ${this.props.entryLabel}`} onClick={this.onAdd} buttonStyle="primary paneHeaderNewButton" marginBottom0>
              + New
            </Button>
          </PaneMenu>
        </IfPermission>
