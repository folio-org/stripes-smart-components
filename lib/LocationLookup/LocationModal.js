import React from 'react';
import PropTypes from 'prop-types';
import Modal from '@folio/stripes-components/lib/Modal';

import LocationForm from './LocationForm';

export default class LocationModal extends React.Component {
  static contextTypes = {
    stripes: PropTypes.shape({
      intl: PropTypes.object.isRequired,
    }).isRequired,
  };

  static propTypes = {
    open: PropTypes.bool,
    onClose: PropTypes.func.isRequired,
    onLocationSelected: PropTypes.func.isRequired,
    resources: PropTypes.shape({
      institutions: PropTypes.shape({
        records: PropTypes.arrayOf(PropTypes.object),
      }),
      campuses: PropTypes.shape({
        records: PropTypes.arrayOf(PropTypes.object),
      }),
      libraries: PropTypes.shape({
        records: PropTypes.arrayOf(PropTypes.object),
      }),
      locations: PropTypes.shape({
        records: PropTypes.arrayOf(PropTypes.object),
      }),
    }).isRequired,
    mutator: PropTypes.shape({
      institutions: PropTypes.shape({
        GET: PropTypes.func,
        reset: PropTypes.func,
      }),
      campuses: PropTypes.shape({
        GET: PropTypes.func,
        reset: PropTypes.func,
      }),
      libraries: PropTypes.shape({
        GET: PropTypes.func,
        reset: PropTypes.func,
      }),
      locations: PropTypes.shape({
        GET: PropTypes.func,
        reset: PropTypes.func,
      }),
    }).isRequired,
  };

  static manifest = Object.freeze({
    institutions: {
      type: 'okapi',
      records: 'locinsts',
      path: 'location-units/institutions',
      accumulate: 'true',
      fetch: false,
    },
    campuses: {
      type: 'okapi',
      records: 'loccamps',
      path: 'location-units/campuses',
      accumulate: 'true',
      fetch: false,
    },
    libraries: {
      type: 'okapi',
      records: 'loclibs',
      path: 'location-units/libraries',
      accumulate: 'true',
      fetch: false,
    },
    locations: {
      type: 'okapi',
      records: 'locations',
      path: 'locations',
      accumulate: 'true',
      fetch: false,
    },
  });

  constructor(props) {
    super(props);
    this.closeModal = this.closeModal.bind(this);
    this.onLocationSelected = this.onLocationSelected.bind(this);
    this.fetchCampuses = this.fetchCampuses.bind(this);
    this.fetchLibraries = this.fetchLibraries.bind(this);
    this.fetchLocations = this.fetchLocations.bind(this);
    this.state = {};
  }

  componentDidMount() {
    this.fetchInstitutions();
  }

  fetchInstitutions() {
    this.props.mutator.institutions.reset();
    this.props.mutator.institutions.GET().then((institutions) => {
      if (institutions.length !== 1) return;
      const [inst] = institutions;
      this.setState({ institutionId: inst.id }); // eslint-disable-line react/no-unused-state
      this.fetchCampuses(inst.id);
    });
  }

  fetchCampuses(institutionId) {
    this.props.mutator.campuses.reset();
    this.props.mutator.libraries.reset();
    this.props.mutator.locations.reset();

    if (!institutionId) return;

    const query = `(institutionId=="${institutionId}")`;
    this.props.mutator.campuses.GET({ params: { query } }).then((campuses) => {
      if (campuses.length !== 1) return;
      const [campus] = campuses;
      this.setState({ campusId: campus.id }); // eslint-disable-line react/no-unused-state
      this.fetchLibraries(campus.id);
    });
  }

  fetchLibraries(campusId) {
    this.props.mutator.libraries.reset();
    this.props.mutator.locations.reset();

    if (!campusId) return;

    const query = `(campusId=="${campusId}")`;
    this.props.mutator.libraries.GET({ params: { query } }).then((libraries) => {
      if (libraries.length !== 1) return;
      const [library] = libraries;
      this.setState({ libraryId: library.id }); // eslint-disable-line react/no-unused-state
      this.fetchLocations(library.id);
    });
  }

  fetchLocations(libraryId) {
    const query = `(libraryId=="${libraryId}")`;
    this.props.mutator.locations.reset();
    return this.props.mutator.locations.GET({ params: { query } }).then((locations) => {
      if (locations.length !== -1) return;
      const [location] = locations;
      this.setState({ locationId: location.id }); // eslint-disable-line react/no-unused-state
    });
  }

  onLocationSelected(location) {
    this.props.onLocationSelected(location);
    this.closeModal();
  }

  closeModal() {
    this.props.onClose();
  }

  render() {
    const { stripes: { intl } } = this.context;
    const { resources } = this.props;
    const institutions = (resources.institutions || {}).records || [];
    const campuses = (resources.campuses || {}).records || [];
    const libraries = (resources.libraries || {}).records || [];
    const locations = (resources.locations || {}).records || [];
    const label = intl.formatMessage({ id: 'stripes-smart-components.ll.selectPermanentLocation' });

    return (
      <Modal onClose={this.closeModal} size="large" open={this.props.open} label={label} dismissible>
        <LocationForm
          initialValues={this.state}
          institutions={institutions}
          campuses={campuses}
          libraries={libraries}
          locations={locations}
          onInstitutionSelected={this.fetchCampuses}
          onCampusSelected={this.fetchLibraries}
          onLibrarySelected={this.fetchLocations}
          onSubmit={this.onLocationSelected}
          onCancel={this.closeModal}
        />
      </Modal>
    );
  }
}
