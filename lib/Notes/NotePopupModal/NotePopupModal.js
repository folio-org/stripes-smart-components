...
// Clear on every page load so the popup re-shows after a browser refresh/login.
...
const setLastShown = (map) => {
  try {
    sessionStorage.setItem(SESSION_KEY, JSON.stringify(map));
  } catch {
    // sessionStorage unavailable (e.g. storage disabled in private browsing)
  }
};

// Call when the user explicitly closes the record detail pane so the popup
// re-shows if the same record is opened again.
// Pass the popUpPropertyName used by the NotePopupModal in that context.
export const resetNotePopupTracking = (popUpPropertyName) => {
  if (!popUpPropertyName) {
    setLastShown({});
    return;
  }
  const map = getLastShown();
  delete map[popUpPropertyName];
  setLastShown(map);
};
...
const NotePopupModal = ({
  ...
  useEffect(() => {
    const lastShown = preventDuplicates ? getLastShown() : {};

    if (entityId && entityId !== lastShown[popUpPropertyName] && !isNotesLoading.current) {
      mutator.NotePopupModal_assignedNotes.GET()
        .then((notes) => {
          const hasPopupNotes = notes.some(note => !!note[popUpPropertyName]);

          if (hasPopupNotes && preventDuplicates) {
            setLastShown({
              ...getLastShown(),
              [popUpPropertyName]: entityId,
            });
          }

          handleGetNotesRequest(notes);
        });

      isNotesLoading.current = true;
    }
  // mutator and handleGetNotesRequest are intentionally omitted: stripesConnect
  // creates a new mutator object on every Redux dispatch, so including them would
  // cause the effect to re-fire after each fetch — re-showing the popup immediately.
  // The effect only needs to re-run when the entity or tracking mode changes.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [entityId, popUpPropertyName, preventDuplicates]);
  ...
  const metaSection = popupNote?.metadata && (
    <ViewMetaData
      metadata={popupNote.metadata}
    />
  );
