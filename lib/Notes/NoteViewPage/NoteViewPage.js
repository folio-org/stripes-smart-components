...
class NoteViewPage extends Component {
  ...
  getMetadata() {
    ...
    const createdByUserId = get(note, 'metadata.createdByUserId');
    const createdByUsername = get(note, 'metadata.createdByUsername');
    const createdDate = get(note, 'metadata.createdDate');
    const lastUpdatedByUserId = get(note, 'metadata.updatedByUserId');
    const lastUpdatedByUsername = get(note, 'metadata.updatedByUsername');
    const lastUpdatedDate = get(note, 'metadata.updatedDate');

    const lastUpdatedBy = createdByUserId === lastUpdatedByUserId
      ? createdByUsername
        ...
        {([pageTitle]) => (
          <TitleManager record={pageTitle}>
            ...
            <ConfirmationModal
              ...
              ariaLabel={intl.formatMessage({ id: 'stripes-smart-components.notes.deleteNote' })}
