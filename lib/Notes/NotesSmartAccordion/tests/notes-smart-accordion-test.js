...
describe('Notes smart accordion', () => {
  ...
  describe('when there are no assigned notes', () => {
    ...
    it('should show empty notes message', () => {
      expect(notesAccordion.noNotesMessagePresent).to.be.true;
    });
  });
  ...
  describe('when clicking on notes list column heading', () => {
    ...
    describe('when clicking on date column heading', () => {
      ...
      it('should sort notes by date', () => {
        expect(notesAccordion.notes(0).date).to.equal('12/4/2018');
        expect(notesAccordion.notes(1).date).to.equal('12/4/2019');
        expect(notesAccordion.notes(2).date).to.equal('12/4/2020');
      });
    });
