...
import { getFullName, isProxyExpired, isUserExpired, isRequestForSponsorNotAllowed } from './proxyUtil';
...
class ProxyForm extends React.Component {
  ...
  renderDisabledRadio(proxy, patron, proxiesMap) {
    let message;

    if (isProxyExpired(patron, proxiesMap)) {
      message = <FormattedMessage id="stripes-smart-components.pm.proxyExpired" />;
    } else if (isUserExpired(proxy)) {
      message = <FormattedMessage id="stripes-smart-components.pm.sponsorExpired" />;
    } else if (isRequestForSponsorNotAllowed(proxy, proxiesMap)) {
  ...
  render() {
    ...
    const isPatronExpired = isUserExpired(patron);
    const isAllProxiesDisabled = proxies.every(proxy => this.renderDisabledRadio(proxy, patron, this.proxiesMap));
    const isAllSelectionOptionsDisabled = isPatronExpired && isAllProxiesDisabled;
    ...
    return (
      <form id="proxy-form" onSubmit={this.handleSubmitLocal}>
        ...
        <Row>
          <Col xs={12}>
            <Field
              ...
              disabled={isPatronExpired}
            />
            {isPatronExpired && <div className={css.error}><FormattedMessage id="stripes-smart-components.pm.proxyExpired" /></div>}
          </Col>
        </Row>
        <br />
        <Row>
        ...
        <Row>
          ...
          <Col xs={3} xsOffset={6}>
            <Button type="submit" fullWidth data-test-continue-button disabled={isAllSelectionOptionsDisabled}>
