...
class ProxyForm extends React.Component {
  ...
  render() {
    ...
    const isProxyUserExpired = isSponsorExpired(patron);
    const allProxiesDisabled = proxies.every(proxy => this.renderDisabledRadio(proxy, patron, this.proxiesMap));
    const allRadiosDisabled = isProxyUserExpired && allProxiesDisabled;
    ...
    return (
      <form id="proxy-form" onSubmit={this.handleSubmitLocal}>
        ...
        <Row>
          <Col xs={12}>
            <Field
              ...
              disabled={isProxyUserExpired}
            />
            {isProxyUserExpired && <div className={css.error}><FormattedMessage id="stripes-smart-components.pm.proxyExpired" /></div>}
          </Col>
        </Row>
        <br />
        <Row>
        ...
        <Row>
          ...
          <Col xs={3} xsOffset={6}>
            <Button type="submit" fullWidth data-test-continue-button disabled={allRadiosDisabled}>
