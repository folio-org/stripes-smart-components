...
class ProxyManager extends React.Component {
  ...
  static propTypes = {
    ...
    sponsorLimitParam: PropTypes.string,
    proxyLimitParam: PropTypes.string,
  };
  ...
  loadSponsorOf() {
    ...
    if (sponsorLimitParam) {
      params.limit = sponsorLimitParam;
    }
    params.query = `query=(userId==${patron.id})`;
    mutator.sponsorOf.reset();
    mutator.sponsorOf.GET({ params });
  }
  ...
  loadProxiesFor() {
    ...
    if (proxyLimitParam) {
      params.limit = proxyLimitParam;
    }
    params.query = `query=(proxyUserId==${patron.id})`;
    mutator.proxiesFor.reset();
    mutator.proxiesFor.GET({ params })
  ...
  loadProxies(proxiesFor) {
    ...
    const params = {};
    const ids = proxiesFor.map(p => p.userId).join(' or ');

    if (!ids) return;
    if (proxyLimitParam) {
      params.limit = proxyLimitParam;
    }
    params.query = `query=id==(${ids})`;
    mutator.proxies.reset();
    mutator.proxies.GET({ params });
