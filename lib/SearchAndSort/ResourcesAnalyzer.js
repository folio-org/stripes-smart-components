...
class ResourcesAnalyzer {
  constructor(resources, logger, resourceName = 'records') {
    this.resources = resources || {};
    this.recordsObj = resources[resourceName] || {};
    this.logger = logger;
  }
  ...
  resultCount() {
    ...
    this.logger.log('analyze', 'ResourcesAnalyzer:resultCount:', res);
    return res;
  }
  ...
  failure() {
    ...
    this.logger.log('analyze-all', 'ResourcesAnalyzer:failure', res);
    return res;
  }

  loaded() {
    const res = this.recordsObj.hasLoaded;
    this.logger.log('analyze-all', 'ResourcesAnalyzer:loaded', res);
    return res;
  }

  successfulMutations() {
    const res = this.recordsObj.successfulMutations || [];
    this.logger.log('analyze-all', 'ResourcesAnalyzer:successfulMutations', res);
    return res;
  }
}
