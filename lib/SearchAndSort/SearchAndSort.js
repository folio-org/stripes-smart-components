  ...
  render() {
    ...
    const newRecordButton = !newRecordPerms ? '' : (
      <IfPermission perm={newRecordPerms}>
        <PaneMenu>
          <Button id={`clickable-new${objectName}`} title={`Add New ${objectNameUC}`} onClick={this.addNewRecord} buttonStyle="primary paneHeaderNewButton" marginBottom0>+ New</Button>
        </PaneMenu>
      </IfPermission>
    );
