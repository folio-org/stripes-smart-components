...
class SearchAndSort extends React.Component {
  static propTypes = {
    ...
    regExpForQuery: PropTypes.string,
  };
  ...
  onSubmitSearch = (e) => {
    ...
    const { validateSearchOnSubmit } = this.props;
    const qindex = locallyChangedQueryIndex || this.props.selectedIndex;
    const isSearchValid = validateSearchOnSubmit?.(locallyChangedSearchTerm);
    const shouldPreventSearch = validateSearchOnSubmit && !isSearchValid;

    if (shouldPreventSearch) return;
