...
class SearchAndSort extends React.Component {
  ...
  constructor(props) {
    ...
    // possibly initialize state with location query parameters
    let localSearch =  '';
    let localQIndex =  '';
    if(syncQueryWithUrl) {
      localSearch = queryParam('query', props);
      localQIndex = queryParam('qindex', props);
    }
  
    this.state = {
      selectedItem: this.initiallySelectedRecord,
      filterPaneIsVisible: storedFilterPaneState === null ? true : !!storedFilterPaneState,
      locallyChangedSearchTerm: localSearch,
      locallyChangedQueryIndex: localQIndex,
      previousQueryString: search,
    };
  ...
  static getDerivedStateFromProps = (props, state) => {
    ...
    if (search !== state.previousQueryString) {
      ...
      if (parsedQuery?.query) {
        nextState.locallyChangedQueryIndex = parsedQuery.query;
      }
