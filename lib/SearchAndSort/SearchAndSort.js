...
class SearchAndSort extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const {
      ...
      pending: prevPending,
      successfulMutations: prevSuccessfulMutations,
     } = makeConnectedSource(prevProps, logger);
    ...
    const {
      ...
      pending,
      successfulMutations,
    } = makeConnectedSource(props, logger);

    // if the results list is winnowed down to a single record, display the record.
    // If there's a single hit, open the detail pane
    const totalCountResult = totalCount();
    if (showSingleResult &&
      totalCountResult === 1 &&
      (this.lastNonNullResultCount > 1 || (prevRecords()[0] !== records()[0]))) {
      this.onSelectRow(null, records()[0]);
    }
    ...
    if (oldStateForFinalSource.records()) {
      ...
      if (finishedResourceNextSM.length > oldStateForFinalSource.successfulMutations().length) {
        ...
        if (newState.successfulMutations()[0]) {
  ...
  //   const oldQuery = queryParam('query', this.props);
  //   const newQuery = queryParam('query', nextProps);
    ...
    //   (this.lastNonNullResultCount > 1 || (oldState.records()[0] !== newState.records()[0]))) {
  ...
  // but only certain user interactions will sync the triggering changes with the internal state, so we derive
  ...
  static getDerivedStateFromProps = (props, state) => {
    ...
    // sync the search field value with the 'query' parameter from window.location
    if (syncQueryWithUrl ) {
      const newQuery = queryParam('query', props);
      if (newQuery !== state.locallyChangedSearchTerm)
      newState.locallyChangedSearchTerm = query;
    }
    ...
    if (newState.keys().length > 0) return newState;
    return null;
  }
