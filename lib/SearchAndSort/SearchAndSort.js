...
/**
 * Okapi returns a result-count of 999999999 whenever a record set has
 * more than 10k rows. IOW, 999999999 doesn't mean 999999999, it means
 * "unknown".
 */
const UNKNOWN_RESULT_COUNT = 999999999;
...
class SearchAndSort extends React.Component {
  ...
  render() {
    ...
    // value comes back as `999999999`, so we use that value as a
    // trigger to display an "unknown total count" value.
    let paneSub;
    if (source.loaded) {
      if (count === UNKNOWN_RESULT_COUNT) {
        paneSub = <FormattedMessage id="stripes-smart-components.searchResultsCountUnknown" />;
      } else {
        paneSub = <FormattedMessage id={messageKey} values={{ count }} />;
      }
    } else {
