...
class SearchAndSort extends React.Component {
  ...
  handleAdvancedSearch = (searchString) => {
    const { advancedSearchIndex } = this.props;
  ...
  splitAdvancedSearchRow = (row) => {
    ...
    return matches.map((match, index) => {
      ...
      const splitIndexAndQueryRegex = /([^=]+)(==\/string|==|=|all)(.+)/g;


      // eslint-disable-next-line no-unused-vars
      const [, option, _match, value] = [...query.matchAll(splitIndexAndQueryRegex)]?.[0]
        .map(i => i.trim())
        .map(i => i.replaceAll('"', ''));
    ...
    return (
      <form onSubmit={this.onSubmitSearch}>
        ...
        <AdvancedSearch
          ...
          splitRow={this.splitAdvancedSearchRow}
        >
          {({ resetRows }) => (
            <Row between="xs">
              {this.renderResetButton(resetRows)}
              {advancedSearchOptions.length && this.renderAdvancedSearchButton()}
            </Row>
          )}
        </AdvancedSearch>
        {
