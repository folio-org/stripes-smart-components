...
class SearchAndSort extends React.Component {
  ...
  componentDidMount = () => {
    ...
    if (
      !moduleContainer.contains(document.activeElement) &&
      this.searchFormRef.current !== null
    ) {
      const firstFocusable = getFirstFocusable(this.searchFormRef.current);
  ...
  renderSearch(source) {
    ...
    return (
      <form onSubmit={this.onSubmitSearch} ref={this.searchFormRef}>
        {renderNavigation()}
        <FormattedMessage
