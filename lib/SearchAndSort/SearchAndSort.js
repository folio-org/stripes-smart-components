...
class SearchAndSort extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    const userQuery = this.queryParam('userQuery', nextProps);
    ...
    if (syncQueryWithUrl && oldQuery !== newQuery) {
      this.setState({ locallyChangedSearchTerm: userQuery || newQuery });
    }
  }
  ...
  onClearSearchQuery = () => {
    ...
    this.transitionToParams({ query: '', userQuery: '' });
  };
  ...
  renderSearch(source) {
    ...
    const userQuery = this.queryParam('userQuery') || '';
    const searchTerm = userQuery || locallyChangedSearchTerm || query;
