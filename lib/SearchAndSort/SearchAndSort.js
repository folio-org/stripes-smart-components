...
class SearchAndSort extends React.Component { // eslint-disable-line react/no-deprecated
  ...
  constructor(props) {
    ...
    this.helperApps = {
      notes: props.stripes.connect(Notes),
  ...
  renderHelper() {
    ...
    const HelperApp = this.helperApps[helper];

    return (
      helper &&
      <Route
        path={`${match.path}/view/:id`}
        render={props => <HelperApp
          stripes={stripes}
          onToggle={() => this.toggleHelper(helper)}
          link={`${moduleName}/${match.params.id}`}
          {...props}
        />}
      />
    );
  ...
  render() {
    const { parentResources, filterConfig, disableFilters, newRecordPerms, viewRecordPerms, objectName, detailProps, stripes } = this.props;
