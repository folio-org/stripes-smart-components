...
class SearchAndSortQuery extends React.Component {
  static propTypes = {
    ...
    retainInternalSearchState: PropTypes.bool,
    searchChangeCallback: PropTypes.func,
    addIsInitialParameterToQuery: PropTypes.bool,
    searchParamsMapping: PropTypes.object,
    setQueryOnMount: PropTypes.bool,
  ...
  static defaultProps = {
    ...
    setQueryOnMount: false,
    addIsInitialParameterToQuery: false,
  }
    ...
    if (props.syncToLocationSearch) {
      ...
      if (props.location.search !== state.previousQuery) {
        ...
        const queryStateFromLocation = {
          searchFields: props.retainInternalSearchState ? {
            query: state.searchFields?.query, // Do not overwrite internal query state unless changed externally
            ...getQueryStateSlice(props.location.search, props.searchParamsMapping)
          } : getQueryStateSlice(props.location.search, props.searchParamsMapping),
        ...
        // rather than update SaSQ's query state first...
        nextState = Object.assign(
          nextState,
          {
            ...queryStateFromLocation,
            changeType: 'external-location',
          }
        );
  ...
  componentDidMount() {
    if (this.props.setQueryOnMount ||
      ...
      this.onSubmitAll(false, 'init.reset', true);
    }
  }
  ...
  onSubmitAll = (dbounce = true, changeType = 'all.submit', isInitialValues = false) => {
    const { addIsInitialParameterToQuery } = this.props;
      ...
      [
        (s) => {
          ...
          const params = {
            ...
            ...filterParams,
            ...s.sortFields,
            ...(addIsInitialParameterToQuery ? { _isInitial: isInitialValues } : {}),
          };
  ...
  onSubmitSearch = (e, changeType = 'search.submit') => {
    ...
    this.internalSetState(
      ...
      changeType,
      [
        (s) => this.debouncedCallQuerySetter({
          ...s.searchFields,
          _isInitial: false,
        }),
      ]
  ...
  resetAll = (submit = true) => {
    ...
    const submitCallback = submit ? () => {
      // call onSubmitAll with `isInitialValues=true` argument to clear _isInitial query parameter
      // otherwise Reset all won't clear the results list
      this.onSubmitAll(false, 'init.reset', true);
    } : noop;
    // this.log('action', 'cleared search');
    this.internalSetState(
  ...
  onSort = (e, meta) => {
    const { maxSortKeys, sortableColumns, sortChangeCallback } = this.props;
    ...
    const sortFields = { sort: sortOrder };
    ...
    this.internalSetState(
      ...
      [
        sortChangeCallback,
        () => this.callQuerySetter(sortFields)
      ]
    );
  };
  ...
  applyFilters = (activeFilters) => {
    ...
    this.callQuerySetter({ ...searchFields, ...paramObject, ...sortFields, _isInitial: false });
  }
