...
class SearchAndSortQuery extends React.Component {
  ...
  constructor(props) {
    ...
    if (props.initialSearch) {
      ...
      const filterString = get(this.initialQuery, 'filters');
      if (filterString) {
        this.initialFilters = {};
        const filterArray = filterString.split(',');
        filterArray.forEach(f => {
          const filter = f.split('.');
          if (this.initialFilters[filter[0]]) {
            this.initialFilters[filter[0]].push(filter[1]);
          } else {
            this.initialFilters[filter[0]] = [filter[1]];
          }
        });
      } else {
        this.initialFilters = {};
