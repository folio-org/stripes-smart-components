...
import { including, MetaSection as MetaSectionInteractor } from '@folio/stripes-testing';

import ViewMetaData from '../ViewMetaData';
import { setupApplication, mount } from '../../../tests/helpers';
import connectStripes from '../../../tests/connectStripes';
import metasection from '@folio/stripes-testing/interactors/metasection';
...
describe('ViewMetaData', () => {
  ...
  describe('MetaData is NOT displayed without proper permissions', () => {
    ...
    it('renders', () => metaSection.exists());

    describe('expanding the metasection information', () => {
      beforeEach(async () => {
        await metaSection.focus();
        await metaSection.clickHeader();
      });
      ...
      it('Given user-id, automated-process details display instead',
        async () => {
          await metaSection.has({ updatedByText: including('Source: Automated process') });
        });
